    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-14 lg:py-20 grid lg:grid-cols-[1.05fr_0.95fr_1fr] gap-8 items-start">
          ...
          {/* RIGHT — Progress path card */}
          <div className="relative">
            <style>{`
              @keyframes dh-shimmer { to { background-position: 200% 0; } }
              @keyframes dh-dot { 0%,100%{ opacity:1; } 50%{ opacity:.35; } }
              @media (prefers-reduced-motion: reduce){ .dh-card, .dh-card * { animation: none !important; } }
            `}</style>
            <div className="dh-card rounded-2xl h-full p-7 lg:p-8 flex flex-col" style={{ background: "linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01))", border: "1px solid rgba(255,255,255,0.08)" }}>
              <div className="flex items-center justify-between">
                <div className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.2em] text-slate-400">
                  <span className="w-1.5 h-1.5 rounded-full" style={{ background: "#00FF88", animation: "dh-dot 2s infinite" }} /> Your progress path
                </div>
                <span className="text-[11px] text-slate-500">8 belts</span>
              </div>

              <h3 className="font-heading text-[26px] lg:text-[30px] tracking-tight leading-tight mt-4">
                Master data skills,<br /><span className="text-[#00FF88]">one belt at a time.</span>
              </h3>
              <p className="text-slate-400 text-sm mt-3 leading-relaxed max-w-sm">
                Solve real SQL, Python, Excel, Power BI and Statistics problems in your browser. Every solve moves you up the belt ladder — from White to Black.
              </p>

              {/* Belt ladder */}
              <div className="mt-7">
                <div className="flex items-center justify-between mb-2.5">
                  <span className="text-xs font-medium text-slate-300">Belt ladder</span>
                  <span className="text-xs text-slate-500">White → Black</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {[["White","#E5E7EB"],["Yellow","#FDE047"],["Orange","#FB923C"],["Green","#4ADE80"],["Blue","#38BDF8"],["Purple","#C084FC"],["Brown","#A16207"],["Black","#4B5563"]].map(([name,color],i) => (
                    <div key={name} className="group relative flex-1" title={name}>
                      <div className="h-2 rounded-full transition-all" style={{ background: color, opacity: i <= 3 ? 1 : 0.28 }} />
                    </div>
                  ))}
                </div>
                <div className="mt-2 flex items-center justify-between text-[11px]">
                  <span className="text-slate-400">Current: <span className="text-white font-medium">Green</span></span>
                  <span className="text-slate-500">Next: Blue</span>
                </div>
              </div>

              {/* Metrics */}
              <div className="grid grid-cols-3 gap-3 mt-7">
                {[["2,000+","Practice questions"],["5","Skill tracks"],["100%","Run in browser"]].map(([n,l]) => (
                  <div key={l} className="rounded-xl px-3 py-3.5 text-center" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                    <div className="font-heading text-xl tracking-tight text-white">{n}</div>
                    <div className="text-[10.5px] text-slate-400 mt-1 leading-tight">{l}</div>
                  </div>
                ))}
              </div>

              {/* Shimmer progress line */}
              <div className="mt-7 h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                <div className="h-full rounded-full" style={{ width: "48%", background: "linear-gradient(90deg,#00FF88,#00D4FF,#00FF88)", backgroundSize: "200% 100%", animation: "dh-shimmer 3s linear infinite" }} />
              </div>
              <div className="text-[11px] text-slate-500 mt-2">48% of the way to your next belt</div>

              <div className="mt-auto pt-7">
                <button onClick={() => nav(user ? "/profile" : "/roadmap")} className="w-full rounded-xl py-3 text-sm font-semibold transition-colors" style={{ background: "#00FF88", color: "#03130f" }} data-testid="hero-panel-start">
                  {user ? "Continue where you left off" : "See the full learning path"}
                </button>
              </div>
            </div>
          </div>
