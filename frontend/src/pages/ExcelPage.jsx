          ...
          <div className="ml-auto flex items-center gap-2" data-testid="workbook-selector">
            ...
            {EXCEL_WORKBOOKS.map((w) => (
              <button key={w.key} onClick={() => setWbKey(w.key)} data-testid={`workbook-${w.key}`}
                ...
                <Sheet className="w-3.5 h-3.5" style={{ color: w.color }} /> {w.name}
        ...
        <div className="w-[360px] shrink-0 border-l border-white/5 bg-[#0F1520] flex flex-col relative" data-testid="excel-challenge-panel">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={() => setShowHint((v) => !v)} variant="outline" size="sm" data-testid="excel-hint" className="h-8 border-[#00D4FF]/40 bg-[#00D4FF]/5 text-[#00D4FF] hover:bg-[#00D4FF]/10"><Lightbulb className="w-3.5 h-3.5 mr-1" /> Hint</Button>
              <Button onClick={() => setShowSolution((v) => !v)} variant="outline" size="sm" data-testid="excel-solution" className="h-8 border-yellow-400/40 bg-yellow-400/5 text-yellow-300 hover:bg-yellow-400/10"><Eye className="w-3.5 h-3.5 mr-1" /> Solution</Button>
            </div>
