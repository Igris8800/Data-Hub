...
export default function PowerBIPage() {
  ...
  const [mode, setMode] = useState(() => loadMode("powerbi"));
  ...
  useActiveChipInView(idx, "powerbi-qdot-");
  ...
  useEffect(() => { saveMode("powerbi", mode); }, [mode]);
  useEffect(() => {
      ...
      <div className="border-b border-white/5 bg-[#0D1117]">
        <div className="max-w-[1600px] mx-auto px-4 py-1.5 flex items-center gap-1 overflow-x-auto" data-testid="powerbi-question-strip">
          {questions.map((q, i) => {
            ...
            return (
              <button key={q.id} onClick={() => jumpTo(i)} title={locked ? `${q.title} · Premium` : learnLock ? `${q.title} · Solve earlier questions first` : q.title} data-testid={`powerbi-qdot-${i}`}
