...
export default function StatsPage() {
  ...
  const [mode, setMode] = useState(() => loadMode("stats"));
  ...
  useEffect(() => { saveMode("stats", mode); }, [mode]);
  useEffect(() => {
  ...
  useEffect(() => {
    const ws = wsKey ? loadWorkspace("stats", wsKey) : null;
  ...
  useEffect(() => { setSolvedIds(localSolvedSet("stats")); }, []);
  useEffect(() => {
    if (!user) return;
    api.get("/progress").then(({ data }) => { const fromServer = hydrateFromAttempts("stats", data.attempts); setSolvedIds((prev) => new Set([...prev, ...fromServer])); setTally(tallyAttempts(data.attempts, "stats")); }).catch(() => {});
  }, [user?.user_id]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (!wsKey) return;
    const t = setTimeout(() => { const prev = loadWorkspace("stats", wsKey) || {}; if ((prev.formula || "") !== formula) saveWorkspace("stats", wsKey, { ...prev, formula }); }, 500);
  ...
  const run = async () => {
    ...
    try {
      ...
      const prevWs = loadWorkspace("stats", wsKey) || {};
      saveWorkspace("stats", wsKey, { ...prevWs, formula, result: r, solved: v.ok || !!prevWs.solved });
      if (v.ok) {
      ...
      if (user) { try { const { data } = await api.post("/progress", { module: "stats", question_id: wsKey, correct: v.ok, difficulty, code: formula }); if (data?.user) setUser(data.user); } catch (e) { void e; } }
      ...
      <div className="border-b border-white/10 bg-[#0F1520]">
        <div className="max-w-[1600px] mx-auto px-4 py-3 flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-md bg-gradient-to-br from-[#B892FF] to-[#00D4FF] flex items-center justify-center text-[#0D1117]"><Code2 className="w-4 h-4" /></div>
            <div><div className="font-heading text-sm tracking-tight">Statistics Lab</div><div className="text-[10px] text-slate-500">by Crazycoder</div></div>
          </div>
