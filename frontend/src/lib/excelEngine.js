...
F.CORREL = (x, y) => { const X = flat(x), Y = flat(y); const a = [], b = []; for (let i = 0; i < Math.min(X.length, Y.length); i++) if (typeof X[i] === "number" && typeof Y[i] === "number") { a.push(X[i]); b.push(Y[i]); } const ma = a.reduce((s, v) => s + v, 0) / a.length, mb = b.reduce((s, v) => s + v, 0) / b.length; let sab = 0, saa = 0, sbb = 0; for (let i = 0; i < a.length; i++) { sab += (a[i] - ma) * (b[i] - mb); saa += (a[i] - ma) ** 2; sbb += (b[i] - mb) ** 2; } return sab / Math.sqrt(saa * sbb); };
...
F.POWER = (a, b) => broadcast(a, b, (x, y) => num(x) ** num(y)); F.MOD = (a, b) => broadcast(a, b, (x, y) => { x = num(x); y = num(y); if (y === 0) throw ERR("#DIV/0!"); return x - y * Math.floor(x / y); });
